...
const app = new Elysia()
    ...
    contentCache = {
      ...
      testimonials: (await db.select().from(testimonials)).map((t: any) => {
        try {
          ...
          return { id: t.id, name: t.name, rating: t.rating, role: parsed.role || '', content: parsed.content || t.content };
        } catch(e) {
          return { id: t.id, name: t.name, rating: t.rating, role: '', content: t.content };
        }
      }),
  ...
  .post("/api/content/testimonials", async ({ body, jwt, cookie: { auth } }: any) => {
    ...
    const contentData = JSON.stringify({ role, content });
    await db.insert(testimonials).values({ name, content: contentData, rating: rating || 5 });
    invalidateCache();
    return { success: true };
  })
  ...
  .post("/api/content/advantages", async ({ body, jwt, cookie: { auth } }: any) => {
    const token = auth.value;
    if (!token) return { success: false, message: "Unauthorized" };
    try { await jwt.verify(token); } catch(e) { return { success: false, message: "Unauthorized" }; }
    
    const { icon, title, description } = body as { icon: string, title: string, description: string };
    if (!title || !description) return { success: false, message: "Title and description required" };
    
    await db.insert(advantages).values({ icon: icon || '⭐', title, description });
    invalidateCache();
    return { success: true };
  })
  .delete("/api/content/advantages/:id", async ({ params, jwt, cookie: { auth } }: any) => {
    const token = auth.value;
    if (!token) return { success: false, message: "Unauthorized" };
    try { await jwt.verify(token); } catch(e) { return { success: false, message: "Unauthorized" }; }
    
    await db.delete(advantages).where(eq(advantages.id, parseInt(params.id)));
    invalidateCache();
    return { success: true };
  })
